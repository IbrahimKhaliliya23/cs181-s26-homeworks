...
\begin{solution}
    ...
    \item
    \begin{enumerate}
        \item In the usual forward-backward notation,
        \[
        \alpha_t(z_t)=p(x_0,\ldots,x_t,z_t), \qquad
        \beta_t(z_t)=p(x_{t+1},\ldots,x_T\mid z_t).
        \]
        If we only condition on observations up to time $t$, then
        \[
        p(z_t\mid x_0,\ldots,x_t)
        = \frac{\alpha_t(z_t)}{\int \alpha_t(z_t)\,dz_t}.
        \]
        So this is the normalized forward message. The operation is called \textbf{filtering}. If we instead conditioned on the full observation sequence $x_0,\ldots,x_T$, then we would use
        \[
        p(z_t\mid x_0,\ldots,x_T) \propto \alpha_t(z_t)\beta_t(z_t),
        \]
        which is smoothing.

        \item Since
        \[
        x_t=z_t+\gamma_t, \qquad \gamma_t\sim N(0,\siggam^2),
        \]
        we have
        \[
        ...
        \[
        p(x_t\mid z_t)
        =\frac{1}{\sqrt{2\pi\siggam^2}}
        \exp\left(-\frac{(x_t-z_t)^2}{2\siggam^2}\right).
        \]

        \item We want the predictive distribution before observing $x_t$. Marginalizing out $z_{t-1}$ gives
        \[
        p(z_t\mid x_0,\ldots,x_{t-1})
        =\int p(z_t\mid z_{t-1})p(z_{t-1}\mid x_0,\ldots,x_{t-1})\,dz_{t-1}.
        \]
        Now
        \[
        p(z_t\mid z_{t-1})=N(z_t;z_{t-1},\sigeps^2)
        \]
        and by assumption
        \[
        p(z_{t-1}\mid x_0,\ldots,x_{t-1})=N(z_{t-1};\mu_{t-1},\sigma_{t-1}^2).
        \]
        Using the Gaussian convolution identity, we get
        \[
        p(z_t\mid x_0,\ldots,x_{t-1})
        =N(z_t;\mu_{t-1},\sigma_{t-1}^2+\sigeps^2).
        \]
        Let
        \[
        \bar{\mu}_t=\mu_{t-1}, \qquad \bar{\sigma}_t^2=\sigma_{t-1}^2+\sigeps^2.
        \]
        Then the predictive distribution is
        \[
        p(z_t\mid x_0,\ldots,x_{t-1})=N(z_t;\bar{\mu}_t,\bar{\sigma}_t^2).
        \]

        \item From parts (b) and (c),
        \[
        p(z_t\mid x_0,\ldots,x_t)
        \propto
        N(x_t;z_t,\siggam^2)N(z_t;\bar{\mu}_t,\bar{\sigma}_t^2).
        \]
        Rewriting the first term as a density in $z_t$,
        \[
        N(x_t;z_t,\siggam^2)=N(z_t;x_t,\siggam^2),
        \]
        so
        \[
        p(z_t\mid x_0,\ldots,x_t)
        \propto
        N(z_t;x_t,\siggam^2)N(z_t;\bar{\mu}_t,\bar{\sigma}_t^2).
        \]
        The product of two Gaussians in the same variable is proportional to another Gaussian. Therefore,
        \[
        p(z_t\mid x_0,\ldots,x_t)=N(z_t;\mu_t,\sigma_t^2),
        \]
        where
        \[
        \sigma_t^2
        =\left(\frac{1}{\bar{\sigma}_t^2}+\frac{1}{\siggam^2}\right)^{-1}
        =\frac{\bar{\sigma}_t^2\siggam^2}{\bar{\sigma}_t^2+\siggam^2},
        \]
        and
        \[
        \mu_t
        =\frac{\siggam^2}{\bar{\sigma}_t^2+\siggam^2}\bar{\mu}_t
        +\frac{\bar{\sigma}_t^2}{\bar{\sigma}_t^2+\siggam^2}x_t.
        \]
        Substituting $\bar{\mu}_t=\mu_{t-1}$ and $\bar{\sigma}_t^2=\sigma_{t-1}^2+\sigeps^2$ gives
        \[
        \mu_t
        =\frac{\siggam^2}{\sigma_{t-1}^2+\sigeps^2+\siggam^2}\mu_{t-1}
        +\frac{\sigma_{t-1}^2+\sigeps^2}{\sigma_{t-1}^2+\sigeps^2+\siggam^2}x_t,
        \]
        and
        \[
        \sigma_t^2
        =\frac{(\sigma_{t-1}^2+\sigeps^2)\siggam^2}{\sigma_{t-1}^2+\sigeps^2+\siggam^2}.
        \]
    \end{enumerate}

    \item The posterior mean $\mu_t$ is a weighted average of the prediction from the past, $\mu_{t-1}$, and the current observation $x_t$. If the observation noise $\siggam^2$ is small, then $x_t$ receives more weight. If the predictive uncertainty $\sigma_{t-1}^2+\sigeps^2$ is small, then the old estimate receives more weight.
\end{enumerate}
...
\begin{solution}
	The policy evaluation and policy/value improvement code is implemented in the notebook. The update I used was
\[
Q(s,a)=R(s)+\gamma\sum_{s'}P(s'\mid s,a)V(s'),
\]
so the policy improvement step chooses
\[
\pi_{\text{new}}(s)=\arg\max_a Q(s,a),
\]
while value iteration updates
\[
V_{\text{new}}(s)=\max_a Q(s,a).
\]

\begin{enumerate}
    \item For policy iteration, the first four iterations are shown below.

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.95\linewidth]{img_output/p2_policy_first4.png}
        \caption{First four policy-iteration updates for $\gamma=0.7$.}
    \end{figure}

    With $\texttt{ct}=0.01$, policy iteration converged in 5 iterations. Using $\texttt{ct}=0.001$ and $\texttt{ct}=0.0001$ also gave convergence in 5 iterations. In this case the tolerance did not change the number of outer policy-iteration steps because the policy itself stabilized exactly after a small number of improvements.

    \item For value iteration, the first four iterations are shown below.

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.95\linewidth]{img_output/p2_value_first4.png}
        \caption{First four value-iteration updates for $\gamma=0.7$.}
    \end{figure}

    With $\texttt{ct}=0.01$, value iteration converged in 25 iterations. With $\texttt{ct}=0.001$, it converged in 31 iterations. With $\texttt{ct}=0.0001$, it converged in 38 iterations. So decreasing the convergence tolerance makes value iteration run for more iterations, since the value function has to get closer to its fixed point before stopping.

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.95\linewidth]{img_output/p2_final_policy_value.png}
        \caption{Final learned value function and policy for policy iteration and value iteration with $\texttt{ct}=0.01$ and $\gamma=0.7$.}
    \end{figure}

    \item The learned policies for different discount factors are shown below.

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.95\linewidth]{img_output/p2_gamma_policies.png}
        \caption{Converged policies for $\gamma\in\{0.6,0.7,0.8,0.9\}$.}
    \end{figure}

    The policies become more future-oriented as $\gamma$ increases. When $\gamma=0.6$, the agent is more myopic and avoids paths whose immediate risk is high, especially around the negative-reward middle column. For $\gamma=0.7$, the policy already starts routing many states toward the large positive state on the right. For $\gamma=0.8$ and $\gamma=0.9$, the repeated long-run value of reaching the $+50$ state dominates the short-term penalties, so more states point east toward the high-reward region. This makes sense because the rewards are infinite-horizon rewards: once the agent reaches a positive state, it can keep receiving that state's reward after taking actions there.

    \item If positive-reward states became terminal, then the policy would no longer be trying to reach a positive state and stay there forever. Instead, each positive reward would be collected only once. For small $\gamma$, the optimal policy would prefer nearby positive rewards because delayed rewards are heavily discounted. For larger $\gamma$, the agent would be more willing to take a longer path to the larger terminal reward, such as the $+50$ state, as long as the risk of hitting negative states is not too large. If the start state itself is treated as positive and terminal, then the modeling choice would need to be adjusted, because otherwise the agent could terminate immediately from the start.

    \item The value of the model-based approach is that we can solve for a policy without physically testing many bad policies on the robot. This is safer, cheaper, and easier to debug than learning directly on the real robot, since the transition probabilities and rewards are explicit. The limitation is that the final policy is only as good as the model. If the slip probabilities, rewards, terminal conditions, or physical constraints are wrong, the computed policy may optimize the Gridworld but not the real task. RL on the robot has a similar issue with reward design: even if it learns from real experience, it can still optimize the specified reward rather than the thing we actually care about.
\end{enumerate}
...
\begin{solution}
I implemented tabular Q-learning with $\epsilon$-greedy exploration in \texttt{hw6\_release.ipynb}. The TD update is
\[
  Q(s,a)\leftarrow Q(s,a)+\alpha\bigl(r+\gamma\max_{a'}Q(s',a')-Q(s,a)\bigr),
\]
equivalent to the $(1-\alpha)$--$\alpha$ averaging form. States are discretized into two indices $(b_x,b_y)$: $b_x=\lfloor\texttt{tree['dist']}/200\rfloor$ and $b_y=\lfloor(\texttt{tree['top']}-\texttt{monkey['top']})/100\rfloor$, clipped to a $7\times 9$ grid (from \texttt{X\_SCREEN}//\texttt{X\_BINSIZE} and \texttt{Y\_SCREEN}//\texttt{Y\_BINSIZE}). $Q$ is a dense NumPy array of shape $(2,7,9)$ over actions $\{\text{swing},\text{jump}\}$. Rewards are the simulator's native rewards (no shaping). Fixed hyperparameters are $\alpha=0.1$, $\gamma=0.9$, and $\epsilon=0.1$.

Problem~2 fixes the global RNG with \texttt{np.random.seed(181)} for Gridworld; Swingy Monkey uses the same NumPy random stream. Before importing the game I call \texttt{np.random.seed(None)} so environment randomness and $\epsilon$-greedy draws are not replayed identically on every full-notebook run.

Training uses \texttt{run\_games(...,\ iters=100)}; final scores are saved as \texttt{hist.npy}. Figure~\ref{fig:swingy-hist} shows score versus epoch (regenerate with the plotting cell after training). Performance is noisy and state aliasing is strong under this coarse binning; meeting the handout's ``score $>50$ once before epoch 100'' would likely require decaying $\epsilon$, extra features (e.g.\ velocity or gravity), or further tuning as suggested in the problem statement.

\begin{center}
\begin{tabular}{lcccc}
\hline
Implementation & $\alpha$ & $\gamma$ & $\epsilon$ & State features \\
\hline
\texttt{Learner} (submitted) & $0.1$ & $0.9$ & $0.1$ fixed & $(b_x,b_y)$ only \\
\hline
\end{tabular}
\end{center}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.82\linewidth]{img_output/swingy_hist}
  \caption{Score vs.\ epoch from \texttt{hist.npy}. Re-run the notebook plotting cell after training if this file is missing.}
  \label{fig:swingy-hist}
\end{figure}
...
\begin{enumerate}
    \item[1a.] Greedy decoding first chooses
    \[
    x_1=A
    \]
    since $0.6>0.4$. Given $x_1=A$, it chooses
    \[
    x_2=A
    \]
    since $0.7>0.3$. Given $(x_1,x_2)=(A,A)$, it chooses
    \[
    x_3=A
    \]
    since $0.6>0.4$. Therefore
    \[
    \mathbf{x}^{\text{greedy}}=(A,A,A).
    \]
    Its joint probability is
    \[
    p(A,A,A)=0.6\cdot 0.7\cdot 0.6=0.252.
    \]

    \item[1b.] Enumerating all 8 sequences:
    \[
    \begin{array}{c|c}
    \text{sequence} & \text{probability} \\
    \hline
    AAA & 0.6\cdot 0.7\cdot 0.6=0.252 \\
    AAB & 0.6\cdot 0.7\cdot 0.4=0.168 \\
    ABA & 0.6\cdot 0.3\cdot 0.5=0.090 \\
    ABB & 0.6\cdot 0.3\cdot 0.5=0.090 \\
    BAA & 0.4\cdot 0.1\cdot 0.5=0.020 \\
    BAB & 0.4\cdot 0.1\cdot 0.5=0.020 \\
    BBA & 0.4\cdot 0.9\cdot 0.05=0.018 \\
    BBB & 0.4\cdot 0.9\cdot 0.95=0.342
    \end{array}
    \]
    Therefore the MAP sequence is
    \[
    \mathbf{x}^\star=(B,B,B), \qquad p(\mathbf{x}^\star)=0.342.
    \]
    This also shows why greedy decoding need not recover the MAP sequence.

    \item[1c.]
    \begin{enumerate}
        \item[(i)] A window $\mathbf{w}$ contains $k$ tokens, and each token has $V$ possible values. Therefore there are
        \[
        V^k
        \]
        possible windows.

        \item[(ii)] Let the new window at time $t+1$ be
        \[
        \mathbf{w}'=(w_2,w_3,\ldots,w_k,a).
        \]
        Then the previous window must have the form
        \[
        \mathbf{w}=(b,w_2,\ldots,w_k)
        \]
        for some $b$ in the vocabulary. The recursion is
        \[
        \delta_{t+1}(w_2,\ldots,w_k,a)
        =\max_{b\in[V]}
        \delta_t(b,w_2,\ldots,w_k)
        p(a\mid b,w_2,\ldots,w_k).
        \]
        We would also store the maximizing $b$ as a backpointer to recover the MAP sequence.

        \item[(iii)] For each time step, there are $V^k$ possible new windows, and for each one we maximize over $V$ possible previous tokens. Thus each layer costs $O(V^{k+1})$, and over $T$ time steps the total cost is
        \[
        O(TV^{k+1}).
        \]
        When $k=1$, this gives
        ...
        O(TV^2),
        \]
        which matches the Viterbi-style bound. When $k=T-1$, this gives
        \[
        O(TV^T),
        \]
        which is exponential and matches brute-force enumeration up to the extra factor of $T$.
    \end{enumerate}
\end{enumerate}
...
\begin{enumerate}
    \item[2a.]
    \begin{enumerate}
        \item[(i)] Just before generating $x_{T+j}$, the model has the original $T$ tokens plus the previous $j-1$ generated tokens. So the sequence length is
        \[
        T+j-1.
        \]
        A full forward pass computes one key for each position, so it requires $T+j-1$ key computations.

        \item[(ii)] Therefore
        \[
        N_{\text{naive}}(T,k)=\sum_{j=1}^k (T+j-1).
        \]
        Compute the sum:
        \[
        N_{\text{naive}}(T,k)=kT+\sum_{j=1}^k(j-1)
        =kT+\frac{k(k-1)}{2}.
        \]
        This is
        \[
        O(kT+k^2)\subseteq O((T+k)^2).
        \]
    \end{enumerate}

    \item[2b.] KV caching works because of causality. In a causal Transformer, the representation at position $t$ only depends on tokens at positions $1,\ldots,t$, not on later tokens. Therefore appending a later token cannot change the old hidden state $h_t$, so the old key $K_t=W_Kh_t$ and value $V_t=W_Vh_t$ remain valid. Under bidirectional self-attention, this breaks down because earlier positions can attend to later positions. If we append a new token, the hidden representations of previous positions may change, so their cached keys and values may no longer be correct.

    \item[2c.]
    \begin{enumerate}
        \item[(i)] With caching, the prefill computes $T$ keys for the original prefix. Then each generated token contributes one new key computation. Hence
        \[
        N_{\text{cached}}(T,k)=T+k.
        \]

        \item[(ii)] The ratio is
        \[
        \frac{N_{\text{naive}}(T,k)}{N_{\text{cached}}(T,k)}
        =\frac{kT+\frac{k(k-1)}{2}}{T+k}.
        \]
        In the regime $k\gg T$, this is approximately
        \[
        \frac{k^2/2}{k}=\frac{k}{2}.
        \]
        Thus the speedup in key computations grows linearly with the generation length, up to constants.
    \end{enumerate}

    \item[2d.] Training parallelizes across positions because we already know the entire target sequence. With teacher forcing, the model can compute the conditional distributions for every position in one masked forward pass. Generation is different because the next token must be sampled before it becomes part of the input for the following step. KV caching does not help much during training because training does not repeatedly recompute the same prefix; each position is computed once in the same forward pass.
\end{enumerate}
...
\begin{enumerate}
    \item[3a.]
    \begin{enumerate}
        \item[(i)] Parallel verification is possible because the draft tokens are already available. The target model can run a causal forward pass on the whole prefix plus all $k$ draft tokens and produce all the conditional distributions in parallel. The same property does not enable parallel generation because, during generation from the target alone, the future tokens are not known yet. We cannot compute the distribution for position $T+i$ until the sampled tokens before it have been chosen.

        \item[(ii)] If $q=p$, then for every draft token,
        \[
        \min\left(1,\frac{p(x)}{q(x)}\right)=1.
        \]
        So every draft token is accepted. However, if the draft and target models are literally identical in size and cost, there is no speedup: we have paid for $k$ sequential draft steps and still pay for a target verification pass. In that case speculative decoding adds overhead. The method is useful when $q$ is much cheaper than $p$ but still close enough to $p$ that many draft tokens are accepted.
    \end{enumerate}
    ...
    \begin{enumerate}
        \item[(i)] If
        \[
        p(x_{T+i}\mid x_{<T+i})\geq q(x_{T+i}\mid x_{<T+i}),
        \]
        then
        \[
        \frac{p(x_{T+i}\mid x_{<T+i})}{q(x_{T+i}\mid x_{<T+i})}\geq 1,
        \]
        so the acceptance probability is
        \[
        1.
        \]
        This makes sense because the draft did not over-sample this token relative to the target. The token is at least as likely under the target as under the draft, so accepting it does not put too much probability mass on it.

        \item[(ii)] If
        \[
        p(x_{T+i}\mid x_{<T+i})\ll q(x_{T+i}\mid x_{<T+i}),
        \]
        then
        \[
        \min\left(1,\frac{p(x_{T+i}\mid x_{<T+i})}{q(x_{T+i}\mid x_{<T+i})}\right)
        \approx \frac{p(x_{T+i}\mid x_{<T+i})}{q(x_{T+i}\mid x_{<T+i})}\ll 1.
        \]
        So the token is usually rejected. This makes sense because the draft model is assigning too much probability to a token that the target model considers unlikely, and accepting it too often would distort the target distribution.
    \end{enumerate}
\end{enumerate}
...
\begin{solution}
	A plausible explanation is that the platform's RL system used a reward function based on engagement, such as watch time, clicks, comments, shares, or probability of returning to the app. If politically extreme content tends to produce stronger emotional reactions, then it can receive higher reward under this objective even if it is socially harmful. The RL agent does not need to explicitly value polarization. It only needs to learn that, for some users, increasingly partisan or outraging content is more likely to keep them scrolling or interacting.

This creates a feedback loop. The user's past behavior becomes part of the state, the recommender chooses content as an action, and the reward signal tells the system which choices increased engagement. If a user engages slightly more with extreme content, the policy may show more of it; after repeated exposure, the user may become more likely to engage with even stronger versions of the same content. So the problem comes from reward misspecification: the app optimizes a measurable proxy for user value, but that proxy can select for content that increases political polarization over time.
