...
\]
equivalent to the $(1-\alpha)$--$\alpha$ averaging form. States are discretized into two indices $(b_x,b_y)$:
\[
  b_x=\left\lfloor \frac{\texttt{tree['dist']}}{200} \right\rfloor,
  \qquad
  b_y=\left\lfloor \frac{\texttt{tree['top']}-\texttt{monkey['top']}}{100} \right\rfloor,
\]
then clipped to a $7\times 9$ grid. The Q-table is a dense NumPy array of shape $(2,7,9)$ over the two actions, swing and jump. I used the simulator's native rewards without extra shaping, with fixed hyperparameters $\alpha=0.1$, $\gamma=0.9$, and $\epsilon=0.1$.

Training was run for 100 epochs using \texttt{run\_games(...,\ iters=100)}, and the episode scores were saved in \texttt{hist.npy}. Figure~\ref{fig:swingy-hist} shows the resulting score trajectory. The policy occasionally survives several trees, but performance remains noisy because the state representation is very coarse: two states with different velocities or gravity values can be mapped to the same $(b_x,b_y)$ bin even though they require different actions. Thus the main limitation of this implementation is state aliasing, not the Q-learning update itself. A stronger version would add velocity and/or an estimate of gravity to the state and use a decaying exploration schedule, but the submitted implementation keeps the learner simple and tabular.
...
\begin{figure}[H]
  ...
  \caption{Swingy Monkey score versus epoch for the submitted Q-learning learner.}
